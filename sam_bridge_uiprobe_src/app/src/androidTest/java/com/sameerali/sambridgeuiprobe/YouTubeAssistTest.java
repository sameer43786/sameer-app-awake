...
public class YouTubeAssistTest {
    ...
    public void runAssist() throws Exception {
        ...
        Log.i(TAG, "READY helper=1.1.0 mode=UiAutomation");
        ...
        try {
            while (SystemClock.elapsedRealtime() - started < MAX_RUNTIME_MS) {
                ...
                muteIfNeeded();
                // Some YouTube variants react to media fast-forward during an identified ad.
                // It is attempted only while an explicit ad marker is present. If ignored,
                // mute remains the safe fallback until ordinary content resumes.
    ...
    private boolean hasAdMarker() {
        ...
        String[] ids = {
                "ad_badge", "ad_attribution", "ad_progress_text", "ad_duration", "ad_info_view",
    ...
    private UiObject2 findActionButton() {
        String[] ids = {
                "skip_ad_button", "skip_ad_button_text", "modern_skip_ad_button", "ad_skip_button",
        ...
        String[] phrases = {
                "Skip ad", "Skip ads", "Skip", "Close ad", "Close", "Dismiss",
    ...
    private boolean hasAnyText(String... values) {
        for (String s : values) {
            if (findByTextOrDesc(s) != null) return true;
        }
        return false;
    }
    ...
    private boolean usable(UiObject2 o) {
        return o != null && belongsToYouTube(o);
    }
    ...
    private String objectLabel(UiObject2 o) {
        ...
        try {
            String d = o.getContentDescription();
    ...
    private void muteIfNeeded() {
        ...
        try {
            ...
            Log.w(TAG, "ERROR mute " + sanitize(String.valueOf(t)));
        }
    }
    ...
    private void restoreVolumeIfNeeded(String reason) {
        ...
        try {
            ...
            Log.w(TAG, "ERROR restore " + sanitize(String.valueOf(t)));
        } finally {
            muted = false;
            originalVolume = -1;
        }
    ...
    private static int parseVolume(String out) {
        ...
        Pattern[] ps = {
                Pattern.compile("(?i)volume is\\s+(\\d+)"),
